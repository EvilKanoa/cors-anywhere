...
function proxyRequest(req, res, proxy) {
  ...
  var appendCookies = JSON.parse(req.headers['x-append-cookies'] || '{}');
  if (appendCookies) {
      Object.keys(appendCookies).forEach(function (cookie) {
          if (!req.headers.cookie) {
              req.headers.cookie = '';
          } else if (req.headers.cookie.length) {
              req.headers.cookie = req.headers.cookie + '; ';
          }
          req.headers.cookie = req.headers.cookie + cookie + '=' + appendCookies[cookie];
      });
  }
...
/**
 * This method parses a response's or request's cookies into a a key-value object.
 * @param headers The response or request headers.
 * @returns {{...[cookieKey]: string}} An object of all set-cookie headers, keyed by cookie name.
 */
function parseCookies(headers) {
  var cookies = JSON.parse(headers['x-forwarded-cookies'] || '{}');
  if (headers['set-cookie2'] && headers['set-cookie2'].length) {
    for (var i = headers['set-cookie2'].length - 1; i >= 0; i--) {
      var cookie2 = headers['set-cookie2'][i].split('=', 2);
      cookies[cookie2[0]] = cookie2[1] || false;
    }
  }
  if (headers['set-cookie'] && headers['set-cookie'].length) {
    for (var j = headers['set-cookie'].length - 1; j >= 0; j--) {
      var cookie = headers['set-cookie'][j].split('=', 2);
      cookies[cookie[0]] = cookie[1] || false;
    }
  }
  return cookies;
}
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  proxyRes.headers['x-forwarded-cookies'] = JSON.stringify(parseCookies(proxyRes.headers));
  ...
  if (statusCode === 301 || statusCode === 302 || statusCode === 303 || statusCode === 307 || statusCode === 308) {
    if (req.headers['x-ignore-redirects']) {
