...
function parseCookies(headers) {
  return []
      .concat(headers['set-cookie2'] || [])
      .concat(headers['set-cookie'] || [])
      .join('; ');
}

function appendCookies(original, cookies) {
    if (cookies) {
        return (original && original.length) ? original + '; ' + cookies : cookies;
    } else {
      return original;
    }
}
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
      ...
      if (locationHeader) {
        ...
        if (statusCode === 301 || statusCode === 302 || statusCode === 303) {
          ...
          if (requestState.redirectCount_ <= requestState.maxRedirects) {
            ...
            // add cookies from previous response
            req.headers.cookie = appendCookies(req.headers.cookie, parseCookies(proxyRes.headers));
  ...
  proxyRes.headers['x-forwarded-cookies'] = parseCookies(proxyRes.headers);
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    // add cookies from x-append-cookies
    if (req.headers.cookie || req.headers['x-append-cookies']) {
      req.headers.cookie = appendCookies(req.headers.cookie, req.headers['x-append-cookies']);
    }
